...
<div class="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(96,165,250,0.15),_transparent_20%),linear-gradient(180deg,_#f8fbff_0%,_#eef4fb_100%)]">
    ...
    <div data-admin-main class="min-h-screen transition-[padding] duration-300 ease-out md:pl-72">
        <header class="border-b border-slate-200/80 bg-white/85 backdrop-blur-xl print:hidden">
            <div class="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
                ...
                <div class="flex flex-wrap gap-3">
                    ...
                    <button type="button" onclick="window.print()" class="rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">
                        Imprimir
                    </button>
                </div>
            </div>
        ...
        <main class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                <section class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
                    <h2 class="text-xl font-semibold text-slate-900">Datos del socio</h2>
                    <dl class="mt-5 grid gap-4 sm:grid-cols-2">
                        <div>
                            <dt class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Nombre</dt>
                            <dd class="mt-1 text-sm text-slate-800">{{ $factura->socio?->persona?->nombre_completo }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Codigo</dt>
                            <dd class="mt-1 text-sm text-slate-800">{{ $factura->socio?->codigo_display }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Sector</dt>
                            <dd class="mt-1 text-sm text-slate-800">{{ $factura->socio?->sector?->nombre }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Tarifa</dt>
                            <dd class="mt-1 text-sm text-slate-800">{{ $factura->socio?->tarifa?->nombre }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Telefono</dt>
                            <dd class="mt-1 text-sm text-slate-800">{{ $factura->socio?->persona?->telefono ?: 'Sin registro' }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Correo</dt>
                            <dd class="mt-1 text-sm text-slate-800">{{ $factura->socio?->persona?->email ?: 'Sin registro' }}</dd>
                        </div>
                    </dl>
                </section>

                <section class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
                    <h2 class="text-xl font-semibold text-slate-900">Datos de facturacion</h2>
                    <dl class="mt-5 grid gap-4 sm:grid-cols-2">
                        <div>
                            <dt class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Periodo</dt>
                            <dd class="mt-1 text-sm text-slate-800">{{ $factura->periodo?->nombre }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Estado</dt>
                            <dd class="mt-1 text-sm text-slate-800">{{ ucfirst($factura->estado) }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Fecha de emision</dt>
                            <dd class="mt-1 text-sm text-slate-800">{{ optional($factura->fecha_emision)->format('d/m/Y') }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Fecha de pago</dt>
                            <dd class="mt-1 text-sm text-slate-800">{{ optional($factura->fecha_pago)->format('d/m/Y') ?: 'Pendiente' }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Consumo</dt>
                            <dd class="mt-1 text-sm text-slate-800">{{ number_format((float) $factura->consumo_m3, 2) }} m3</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Medidor</dt>
                            <dd class="mt-1 text-sm text-slate-800">{{ $factura->lectura?->medidor?->numero_serie ?: 'Sin medidor' }}</dd>
                        </div>
                    </dl>
                </section>
            </div>
            ...
            <section class="mt-6 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
                <div class="flex items-center justify-between gap-4">
                    <h2 class="text-xl font-semibold text-slate-900">Resumen de montos</h2>
                ...
                <div class="mt-5 grid gap-4 md:grid-cols-6">
                    <div class="rounded-2xl bg-slate-50 p-4">
                        <p class="text-xs uppercase tracking-[0.2em] text-slate-400">Consumo</p>
                        <p class="mt-2 text-lg font-semibold text-slate-900">Bs {{ number_format((float) $factura->monto_consumo, 2) }}</p>
                    </div>
                    <div class="rounded-2xl bg-slate-50 p-4">
                        <p class="text-xs uppercase tracking-[0.2em] text-slate-400">Cargo fijo</p>
                        <p class="mt-2 text-lg font-semibold text-slate-900">Bs {{ number_format((float) $factura->cargo_fijo, 2) }}</p>
                    </div>
                    <div class="rounded-2xl bg-slate-50 p-4">
                        <p class="text-xs uppercase tracking-[0.2em] text-slate-400">Recargo</p>
                        <p class="mt-2 text-lg font-semibold text-slate-900">Bs {{ number_format((float) $factura->recargo_mora, 2) }}</p>
                    </div>
                    <div class="rounded-2xl bg-slate-50 p-4">
                        <p class="text-xs uppercase tracking-[0.2em] text-slate-400">Subtotal</p>
                        <p class="mt-2 text-lg font-semibold text-slate-900">Bs {{ number_format((float) $resumenCobro['subtotal'], 2) }}</p>
                    </div>
