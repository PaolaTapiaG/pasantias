...
@section('title', 'Editar tarifa - EPSAS')
    ...
    <div data-admin-main class="min-h-screen transition-[padding] duration-300 ease-out md:pl-72">
        <header class="border-b border-slate-200/80 bg-white/85 backdrop-blur-xl">
            <div class="mx-auto flex max-w-4xl items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
                <div>
                    ...
                    <h1 class="mt-1 text-2xl font-bold tracking-tight text-slate-900">Editar tarifa</h1>
                </div>
                <a href="{{ route('admin.tarifas.index') }}" class="rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">
                    Volver
                </a>
            </div>
        </header>
        ...
        <main class="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
            ...
            @include('tarifas.partials.form', [
                'action' => route('admin.tarifas.update', $tarifa),
                'method' => 'PUT',
                'tarifa' => $tarifa,
