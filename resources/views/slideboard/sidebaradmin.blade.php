@php
    ...
    if ($user && $user->hasRole('secretaria')) {
        echo view('slideboard.sidebarsec')->render();
        return;
    }

    $companySettings = $sharedCompanySettings ?? [];
    $adminUser = $sharedAuthUser;
        ...
        <div class="flex items-center justify-between gap-3 px-2">
            <div class="flex min-w-0 items-center gap-3">
                @if (!empty($companySettings['company_logo']))
                    <img src="{{ asset($companySettings['company_logo']) }}" alt="Logo empresa" class="h-12 w-12 shrink-0 rounded-2xl bg-white/15 p-1 object-contain shadow-inner shadow-white/10">
                @else
